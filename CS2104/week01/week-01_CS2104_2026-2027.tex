...
\author{
    \textbf{Sim Yue Yang} (Student ID, yueyangsim02@gmail.com)
    % TODO
}
...
\item
  The point of the exercise was to copy-paste the question into google.
...
\item
  To identify the value of the shifted nested sum.

\item
  By recursion and an OEIS lookup, giving the Catalan numbers.
...
\item
  To characterize a nested sum whose bounds are of the form $1 - i$.
...
\item
  To identify the value of this nested sum.

\item
  By recursion producing $f(N-1) + f(N-2)$, giving the Fibonacci numbers.

\item
  The Fibonacci recurrence arising from the summation bounds.
...
\item
  To characterize nested sums with constant bounds decreasing from $N$ to 0.

\item
  By multiplying the layer sizes, tabulating the outputs, and matching against the OEIS.
...
\item
  To identify the value of these nested sums.

\item
  By taking the product of the layer sizes, giving $(N + 1)!$.

\item
  Factorial as a product of counts.
...
\item
  To characterize nested sums with constant bounds increasing from 0 to $N$.
...
\item
  By taking the product of the layer sizes, giving $(N + 1)!$ as in Exercise 10.

\item
  The order of the bounds does not change the product.
...
\item
  Where are we in the semester and what is the general topic addressed in this
  report?
